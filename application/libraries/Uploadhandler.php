...
class UploadHandler
{
    ...
    protected function get_unique_filename($file_path, $name, $size, $type, $error,$index, $content_range) {
        ...
        $diganti = array(' ');
        // $name = strtolower(str_replace($diganti, '_', $name));
        return $name;
    }
    ...
    public function post($print_response = true) {
        ...
        // pre($this->options['param_name']); exit;
        ...
        // pre($response);
        $this->insert_db($response);
        return $this->generate_response($response, $print_response);
    }

    public function insert_db($response)
    {
        if(!empty($response)):
            foreach($response as $row):
                $this->CI->db->query("INSERT INTO {PRE}files (name, size, type) VALUES ('".$row[0]->name."', '".$row[0]->size."', '".$row[0]->type."') ");
            endforeach;
        endif;
    }
