  ...
  */
  class Datatables
  {
    ...
    */
    protected function get_ordering()
    {
      ...
      elseif($this->ci->input->post('sColumns'))
        ...
        //$this->debuger = $mColArray[1];

        for($i = 0; $i < intval($this->ci->input->post('iSortingCols')); $i++){
            //$this->debuger = $mColArray[intval($this->ci->input->post('iSortCol_' . $i-1))];
            ...
            {
                //$ccd = intval($this->ci->input->post('iSortCol_' . $i));
                $ccd = (int) $this->ci->input->post('iSortCol_' . $i);
                $ddf = $ccd - 1;
                $ddf = ($ddf=='-1') ? '0' : $ddf;
                //$this->debuger = $ddf;

                $this->ci->db->order_by($mColArray[$ddf], $this->ci->input->post('sSortDir_' . $i));
            }
        }
    }
    ...
    */
    protected function get_filtering()
    {
        if($this->check_mDataprop()){
            ...
            $mColArray = explode(',', $this->ci->input->post('sColumns'));
            // $this->debuger = json_encode($mColArray);
        } else {
            $mColArray = $this->columns;
            //$this->debuger = json_encode($mColArray);
        }
    ...
    */
    ...
    {
        $data = $this->ci->db->get();
        $this->debuger .= $this->ci->db->last_query();
        return $data;
    }
    ...
    */
    ...
    {
      //$ccv = $this->columns;
      array_unshift($this->columns , 'urut');
      //pre($ccv);

      $aaData = array();
      $rResult = $this->get_display_result(); //pre($rResult);
      $iTotal = $this->get_total_results();
      $iFilteredTotal = $this->get_total_results(TRUE);

      $iStart = $this->ci->input->post('iDisplayStart');
      $urut = ($iStart)? $iStart : 0;

      foreach($rResult->result_array() as $row_key => $row_val)
      {
        $urut++;
        //$row_val['ser'] = $urut;
        array_unshift($row_val, $urut);
    ...
    */
    protected function exec_replace($custom_val, $row_data)
    {
     ...
     // pre($custom_val['content']);

      //tuyul
      $rest = substr($custom_val['content'], 0,2);
      //pre($rest);
      if($rest=='/*'){
        $komen = explode('*/', $custom_val['content']);
        $func = substr($komen[0], 2, strlen($komen[0])-2 );

        $fnc = explode('|', $func);
        //pre($fnc[1]);

        $satu = (isset($fnc[1]) && isset($fnc[1]) !='' ) ? $fnc[1] : '';
        $dua = (isset($fnc[2]) && isset($fnc[2]) !='' ) ? $fnc[2] : '';
        $tiga = (isset($fnc[3]) && isset($fnc[3]) !='' ) ? $fnc[3] : '';
        $empat = (isset($fnc[4]) && isset($fnc[4]) !='' ) ? $fnc[4] : '';

        return $fnc[0]($custom_val['content'], $satu, $dua, $tiga, $empat);
      }elseif($rest=='^|'){
        $all = substr($custom_val['content'], 2, strlen($custom_val['content']));
        $fnc = explode('|', $all);
        ...
        return $fnc[0]($satu, $dua, $tiga, $empat);
      } else {
          //pre($custom_val);
