...
class finance_customer_data_dismantle extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->js_inject .= $this->load->view('finance_customer_data_dismantle/js', $data, TRUE);
        ...
        $data['insert_view'] = $this->load->view('finance_customer_data_dismantle/form', $data, TRUE);
        //$data['delete_view'] = $this->load->view('finance_customer_data_dismantle/delete', $data, TRUE);

        $konten = $this->load->view('finance_customer_data_dismantle/index', $data, TRUE);
    ...
    {
        echo json_encode($this->finance_customer_data_dismantle->select());
    }
    ...
    {
        $this->finance_customer_data_dismantle->select_detail_ref($this->uri->segment(3));
    }

    function get_coa()
    {
        if (!empty($this->input->post('searchTerm'))) {
            $list = $this->finance_customer_data_dismantle->select_autocomplite_coa_id($this->input->post('searchTerm'));
        } else {
            $list = $this->finance_customer_data_dismantle->select_autocomplite_coa_idfull();
        }
        $data = array();
        foreach ($list as $row) {
