...
class Report extends MY_Controller {
	...
	{
		$modul = $this->report->info_modul($what);
		$set_ui = $this->report->set_ui($what);
		...
		$this->js_inject .= $this->load->view('report/js_table', $data, TRUE);
		$this->js_inject .= $this->load->view('report/js', $data, TRUE);

		$tabs = $this->report->tabs($what);
		$data['modal_view'] = $this->load->view('report/modal', $data, TRUE);

		$main_index_view = (empty($tabs)) ? 'index' : 'index_tab';
		$konten = $this->load->view('report/'.$main_index_view, $data, TRUE);
