...
function get_tabel_data(){
	...
	$("#tableData").flexigrid({
		...
		colModel : [{ display : 'no',	name : 'no', width : 30, sortable : false, align : 'right' },
					{ display : 'Nama',	name : 'a.name', width : 200, sortable : true, align : 'left' },
					{ display : 'atas nama',	name : 'a.account_name', width : 200, sortable : true, align : 'left' },
					{ display : 'no referensi',	name : 'a.account_number', width : 200, sortable : true, align : 'left' },
		...
		success: function(html){
			if(html != ''){
				$.each(html, function(i,n){
					$('#id').val(id);
					$('#name').val(n["name"]);
					$('#account_name').val(n["account_name"]);
					$('#account_number').val(n["account_number"]);
					$('#modal_dialog_formulir').modal('show');
					unBlockUI();
				});
			}else{
