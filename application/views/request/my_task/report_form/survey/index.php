...
<form id="form_survey_report" action="<?php echo base_url(); ?>xhr/task_report/create/<?php echo $task_id; ?>" method="post">
...
<?php
	...
	$this->db->select('task.id,
		task_report_presurvey.status_coverage,
		task_report_presurvey.koordinat,
		task_report_presurvey.jarak_opd_pelanggan,
		task_report_presurvey.estimasi_waktu_pengerjaan,
		task_report_presurvey.jenis_tower,
		task_report_presurvey.tinggi_tower,
		task_report_presurvey.estimasi_biaya,
		task_report_presurvey.nama_vendor,
		task_report_presurvey.note
		');
	$this->db->join('task_report_presurvey', 'task.id = task_report_presurvey.task_id', 'left');
	$pre_survey_data = $this->db->get('task')->row_array();
	...
	$options['tabs'][] = array(
			'label'         => 'Data Teknis',
			'id'            => 'detail_teknis',
			'content'       =>  $this->load->view('request/my_task/report_form/survey/data_teknis', '', TRUE)
		);

	$options['tabs'][] = array(
			'label'         => 'primary Link',
			'id'            => 'primary_link',
			'content'       => $this->load->view('request/my_task/report_form/survey/primary_link', '', TRUE)
		);

	$options['tabs'][] = array(
			'label'         => 'Secondary Link',
			'id'            => 'secondary_link',
			'content'       => $this->load->view('request/my_task/report_form/survey/secondary_link', '', TRUE)
		);


	$options['tabs'][] = array(
			'label'         => 'Kebutuhan Perangkat (BOQ)',
			'id'            => 'perangkat_boq',
			'content'       =>  '<div id="boq_div">BOQ</div>'
		);
...
<script type="text/javascript">
	...
	getajax('<?php echo base_url(); ?>xhr/task_item/index/task_boq/0/survey_boq/boq_div/ts_boq', 'boq_div');
