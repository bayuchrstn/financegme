<?php
	// pre($task_detail);
	// pre($report_detail);
	// pre($install_link_secondary);

	$prefix  = 'secondary_link';
	$default_value = array();
	$forms_report = $this->ui->forms('task_report', $default_value, $prefix);
	echo $forms_report['secondary_link_type'];
	$selected_jenis_secondary_link = ( isset($install_link_secondary['jenis']) ) ? $install_link_secondary['jenis'] : 'tidak_ada';
...
<script type="text/javascript">
    $(document).ready(function(){
        set_option('<?php echo base_url(); ?>select_option/request/my_task/jenis_secondary_link_install', 'secondary_link_type_secondary_link', '<?php echo $selected_jenis_secondary_link; ?>');
    ...
    function change_secondary_link_type(jenis_link)
	{
		$.ajax({
			type:'GET',
			// url: '<?php echo base_url(); ?>xhr/task_report/link_type_picker/'+jenis_link,
			url: '<?php echo base_url(); ?>xhr/task_report/link_type_picker/install/secondary/'+jenis_link+'/<?php echo $rid; ?>',
