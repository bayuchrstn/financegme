    ...
    function update(id)
    {
        ...
        $.ajax({
            ...
            success: function(res) {
				...
				$('#modal_request_update #modal_title_custom').html('Approval Pengadaan Barang');
				$("#form_<?php echo $modul['code']?>_update").attr('action', '<?php echo base_url(); ?>xhr/view_pengadaan_barang/approval');
				$("#form_<?php echo $modul['code']?>_update #id_update").val(response.id);
				$("#info_approved_update").load("<?php echo base_url(); ?>pengajuan_barang/show/"+response.id+"/echo");
				set_option('<?php echo base_url(); ?>select_option/approval', 'approval_status_update', '');
