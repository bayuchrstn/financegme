<script type="text/javascript">
	...
	//question check
	function ticketQuestion(checkbox) {
		if ( $(checkbox).is(':checked') ) {
			//request ticket_questions
			var action = '<?=base_url();?>master/data/ticket_questions';
			$.ajax({
				type: 'GET',
				url : action,
				success: function(res){
					var response = $.parseJSON(res);
					var append = '<div id="tkt_qst">';

					append += '<table class="table table-bordered table_data_info">';

					for (var i = 0; i < response.data.length; i++) {
						input = '<input type="text" name="question[]" class="form-control">';
						input += '<input type="hidden" name="id_question[]" value="'+response.data[i].id+'" >';
						append += '<tr>';
						append += '<td valign="top" width="200">'+response.data[i].note+'</td>';
						append += '<td valign="top">'+input+'</td>';
						append += '</tr>';
					}
					...
					$('#modal_asearch_ticket_question').append(append);
				}
			});
		} else {
			$('#tkt_qst').remove();
    ...
    function input()
    {
		set_option('<?php echo base_url(); ?>select_option/request/ticket/ticket_status', 'status_create', 'open');
		...
		var extra_question = '<div class="form-group"><label class="checkbox-inline"><input type="checkbox" name="ticket_question" id="ticket_question" onchange="ticketQuestion(this);"> Pertanyaan Ticket</label></div>';
		$('#modal_asearch_ticket_question').empty().append(extra_question);
	...
	function update(id)
    {
        ...
        $.ajax({
            ...
            success: function(res) {
				...
				set_option('<?php echo base_url(); ?>select_option/request/ticket/ticket_status', 'status_view_ticket', response.status);
