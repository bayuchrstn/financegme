...
<tr class="text-uppercase text-center text-size-large">
    <th colspan="<?php echo $n; ?>">NERACA (<?php echo $this->m_global->cek_name_regional($this->session->userdata('scope_area')); ?>)</th>
</tr>
<tr class="text-uppercase text-center text-size-large">
	<th>AKUN</th>
<?php
...
<?php
	...
	$this->db->where_in('kelompok', $kelompok);
	$this->db->order_by('id');
	$q = $this->db->get();
	$data = $q->result_array();
	//$this->finance_accounting_report_neraca_period->looping_child_label(1, $data, $tanggal_awal, $tanggal_akhir);
	$this->finance_accounting_report_neraca_period->akun_label(1, $data);
?>
</table>
</td>
<?php
...
//$begin_start = date($begin_start, strtotime("-1 months"));
$begin_start = strtotime($begin_start.' -1 months');
$begin_start = date('Y-m-01', $begin_start);
$begin_finish = date("Y-m-t", strtotime($begin_start));
/*
echo '<td style="padding:0; border:0;">
<table class="table datatable-ajax table-striped table-xxs text-size-small text-nowrap table-bordered">';
$this->finance_accounting_report_neraca_period->looping_child_label(1, $data, $begin_start, $begin_finish);
echo '</table>
</td>';
*/
foreach($date_period as $date_period_no => $date_period_dt){
	echo '<td style="padding:0; border:0;">
<table class="table datatable-ajax table-striped table-xxs text-size-small text-nowrap table-bordered">';
	//$this->finance_accounting_report_neraca_period->looping_child_label_current(1, $data, $date_period_dt->format('Y-m-01'), $date_period_dt->format('Y-m-t'));
	$this->finance_accounting_report_neraca_period->looping_child_label(1, $data, $date_period_dt->format('Y-m-01'), $date_period_dt->format('Y-m-t'));
	echo '</table>
</td>';
}
/*
...
echo '<td style="padding:0; border:0;">
<table class="table datatable-ajax table-striped table-xxs text-size-small text-nowrap table-bordered">';
$this->finance_accounting_report_neraca_period->looping_child_label(1, $data, $ending_start, $ending_finish);
echo '</table>
</td>';
*/
?>
