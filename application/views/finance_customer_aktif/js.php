<script type="text/javascript">
			...
			success: function(html) {
				if (html != '') {
					$.each(html, function(i, n) {
						$('#id').val(id);
						$('#id_site').val(n["id_site"]);
						$('#npwp').val(n["npwp"]);
						$('#email').val(n["email_cust"]);
						$('#alamat').val(n["alamat2"]);
						$('#materai').val(n["materai"]).change();
						$('#inv_int').val(n["installasi"]).change();
						$('#modal_dialog_formulir').modal('show');
						unBlockUI();
					});
				} else {
	...
	function cetak(elem) {
		var id = $(elem).attr("id");
		window.open(<?php echo json_encode(base_url()); ?> + 'finance_invoice_customer/print_selected' + '?inv=' + id, '_blank ');
	};
</script>