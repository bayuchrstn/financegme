		...
		success: function(html){
			if(html != ''){
				$.each(html, function(i,n){
					$('#id').val(id);
					$('#detail_customer').html(n["detail_customer"]);
					$('#status_active').val(n["status_activenya"]);
					$('#nonaktif_date').val(n["nonaktif_date"]);
					$('#nonaktif_reason').val(n["nonaktif_reason"]);
					//$('#tanggal_billing').val(n["tanggal_billing"]);
					$('#modal_dialog_formulir').modal('show');
					unBlockUI();
				});
			}else{
