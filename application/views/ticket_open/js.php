...
function input_data(){
	$('#formulir_modal').clearForm();
	$('#timeline,#ref_id,#detail_question').html('');
...
function closed_data(id){
	...
	if(confirmProses('Yakin ingin close ticket?') == true) {
		$.ajax({
			type:'GET', 
			url: pageUri+'closed_data/'+id, 
			beforeSend: function(){
				blockUI();
			},
			success: function(response){
...
function change_question(id){
	...
	$.ajax({
		type:'GET', 
		url: pageUri+'change_question/'+id+'/'+id_ticket,
		beforeSend: function(){
			blockUI();
		},
		success: function(html){
			$('#detail_question').html(html);
			unBlockUI();
		}
	});
}
