...
<style>
	...
	.uk-h2,h2{font-size:24px;line-height:30px}.uk-h3,h3{font-size:18px;line-height:24px}
</style>
<div class="invoice" style="font-family:roboto">
	<?php
				...
				foreach ($get as $row) {
					...
					if ($arpost->flag_dp == 1) {
						if ($arpost->flag_installasi == 1) {
							...
							$serv = $this->Finance_model->get('order_service', $sum_transaksi->id_order_service)->row();
							$id_serv = isset($serv->id_serv) ? $serv->id_serv : '';
							$lbl_serv = '';
							if ($id_serv != '') {
								// $get_serv = $this->Finance_model->get('ms_layanan',$serv->id_serv)->row();
								$get_serv = $this->Finance_model->get_from_arr('ms_layanan', array('id' => $serv->id_serv))->row();
								...
								if (!empty($arpost->label_note)) {
									$lbl_serv = $get_serv->label . ' ' . $row->label_note;
								} else {
									$lbl_serv = $get_serv->label;
								}
							}
							...
							echo '<tr>
													...
													<td class="td_small" style="text-align:center;vertical-align:middle;border-left: 1px solid black;">Rp  <span class="text-right" style="float: right;">' . $this->Kamus_model->uang($sum_transaksi->total) . '</span></td>
													<td class="td_small" style="text-align:center;vertical-align:middle;border-left: 1px solid black;">Rp  <span class="text-right" style="float: right;">' . $this->Kamus_model->uang($sum_transaksi->total) . '</span></td>
											</tr>';
