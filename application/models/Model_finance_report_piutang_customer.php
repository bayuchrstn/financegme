<?php
class Model_finance_report_piutang_customer extends CI_Model
{
	...
	function get_data_table()
	{
		...
		if ($this->input->post('searchTanggal') == '1') {
			$kondisi .= "WHERE (z.`tanggal_invoice` between '" . $this->input->post('searchDateFirst') . "' and '" . $this->input->post('searchDateFinish') . "')";
		}
		...
		// }
		elseif ($this->input->post('searchTanggal') == '3') {
			$kondisi .= "WHERE (z.`tanggal_invoice` <= '" . $this->input->post('searchDateFinish2') . "')";
		...
		if ($this->input->post('searchppn') != '') {
			if ($this->input->post('searchppn') == 1) {
				$kondisi .= ' AND (z.`ppnnya`= 1 OR z.`ppnnya`=3)';
			} else {
				$kondisi .= ' AND z.`ppnnya`= 2';
			}
		}
		...
		c.`phonewakil`,
		DATEDIFF('$tanggal', a.`tanggal_invoice`) AS aging,
		d.`ppn` AS ppnnya
	  FROM
		erp_gmedia.`arpost` a
		...
		ON a.`id_order`=d.`id`
    WHERE a.`lunas`=0 AND a.`status_invoice` > 1 AND a.`status`=1 AND a.`id_order` IS NOT NULL
		  UNION ALL
		  SELECT
		a.*,
		DATE_FORMAT(a.`tanggal_invoice`, '%d-%m-%Y') AS date_invoicenya,
		...
		ON c.`id_cust`=d.`id`
		WHERE a.`lunas`=0 AND a.`status_invoice` > 1 AND a.`status`=1 AND a.`merge_type`=1
		GROUP BY a.`id`) z $kondisi
