<?php
class Model_finance_master_kat_gl extends CI_Model {
	...
	function get_data_table()
	{
        ...
        $this->db->from('finance_master_kat_gl a');
		$this->db->group_start();
	...
	function insert(){
		...
		$result=$this->db->insert('finance_master_kat_gl', $data);	
		if($result==true)
		{
			$msg = 1;
		}
	...
	function update(){
		...
		$this->db->where('id', $this->input->post('id'));
		$result=$this->db->update('finance_master_kat_gl', $data);	
		if($result==true)
		{
			$msg = 1;
		}
	...
	{
		$this->db->where('id', $id);
		$result=$this->db->delete('finance_master_kat_gl');
		if($result==true)
		{
			$msg = 1;
		}
