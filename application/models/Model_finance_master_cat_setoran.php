<?php
class Model_finance_master_cat_setoran extends CI_Model {
	...
	function get_data_table()
	{
        ...
        $this->db->from('finance_master_cat_setoran a');
		$this->db->group_start();
	...
	function insert(){
		...
		$result=$this->db->insert('finance_master_cat_setoran', $data);	
		if($result==true)
		{
			$msg = 1;
		}
	...
	function select(){
		...
		$q = $this->db->get("finance_master_cat_setoran");
		return $q->result();
	}
	...
	function update(){
		...
		$this->db->where('id', $this->input->post('id'));
		$result=$this->db->update('finance_master_cat_setoran', $data);	
		if($result==true)
		{
			$msg = 1;
		}
	...
	{
		$this->db->where('id', $id);
		$result=$this->db->delete('finance_master_cat_setoran');
		if($result==true)
		{
			$msg = 1;
		}
