<?php
class Model_request_out extends CI_Model {
	...
	function widget($mode, $modul)
	{
        ...
        $arr = array();
        switch ($mode) {
            case 'dashboard_ro_request':
                $this->db->order_by('id', 'desc');
                $this->db->where('task_category', 'request_out');
	...
	{
		$arr = array();
		$arr['save_item_out'] = $this->save_item_out($task_id);
		return $arr;
	}
	...
	function save_item_out($task_id)
    {
        $arr = array();
	...
	function save_item_out_old($task_id)
    {
		$arr = array();
		$cart = $this->cart->contents();
	...
	function save_item_outddd($task_id)
    {
		$arr = array();
		$cart = $this->cart->contents();
		$key = $this->cart_to_key($cart);
		...
		$key_comma = arr_to_comma($key);
		if(empty($data)):
            $sql_delete = "DELETE FROM {PRE}task_item_out WHERE task_id='".$task_id."' ";
            $this->db->query($sql_delete);
        else:
			$sql_delete = "DELETE FROM {PRE}task_item_out WHERE task_id='".$task_id."' AND item_id NOT IN (".$key_comma.")";
			// exit;
            $this->db->query($sql_delete);
			...
			foreach($data as $key=>$val):
                ...
                $sql_cek = "SELECT * FROM {PRE}task_item_out WHERE task_id='".$task_id."' AND item_id='".$val['id']."' ";
                ...
                //jika ada maka diupdate
					...
					// );
					// // pre($data);
					// $this->db->where('kits_id', $kits_id);
					// $this->db->where('item', $val['item']);
					// $this->db->where('item_id', $val['item_id']);
					// $this->db->update('kits_items', $data);
                endif;
