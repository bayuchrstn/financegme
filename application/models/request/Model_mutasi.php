...
class Model_mutasi extends CI_Model {
	...
	function tabs()
	{
        ...
        $arr['selected'] = array(
			...
			'table_columns' => array(
				...
				array('label'   => 'tanggal Permintaan'),
				array('label'   => 'sales'),
	...
	{
		$params = array();
		$this->request->task_ext($task_id, 'task_mutasi', $params);
	}
	...
	function select_option($mode='customer')
	{
		...
		switch ($mode) {
            ...
            case 'type_mutasi' :
                $arr['upgrade'] = 'Upgrade';
                $arr['downgrade'] = 'Downgrade';
                $arr['berhenti'] = 'Berhenti';
                $arr['relokasi'] = 'Relokasi';
			break;
	...
	function build_select_option()
	{
		...
		$arr['type_mutasi'] = $this->select_option('type_mutasi');
		return $arr;
	}
}
