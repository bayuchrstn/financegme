...
class Model_ticket extends CI_Model {
	...
	function tabs()
	{
        ...
        $status_ticket = $this->master->arr('ticket_status');
        // pre($status_ticket); exit();

        if(!empty($status_ticket)):
            foreach($status_ticket as $key=>$val):
                // pre($key);
                $selected_tab = ($key=='open') ? 'selected' : '';
                if ($key=='open' || $key=='close') :
        		$arr[$selected_tab] = array(
        			'name' => $val,
        			'code' => $key,
        			'table_columns' => array(
        				array('label'   => '#', 'width'=>'5'),
	...
	function detail($task_id)
	{
		...
		if(!empty($task_detail)):
			...
			if ( substr($arr['status'], 0, 15)=='ticket_helpdesk' && !empty($arr['note']) ) {
				$ticket_question = unserialize($arr['note']);
				foreach ($ticket_question as $row) {
					$master = $this->db->where('id', $row['q'])->get('master')->row_array();
					$arr['ticket_question'][] = array(
						'id'	=> intval($master['id']),
						'tanya'	=> $master['note'],
						'jawab'	=> $row['a'],
					);
	...
	function select_option($mode='customer')
	{
		...
		switch ($mode) {
			...
			case 'ticket_status' :
				...
				$data2 = $this->master->arr('ticket_helpdesk_status');
				if(!empty($data)):
	...
	function build_select_option()
	{
		...
		$arr['ticket_status'] = $this->select_option('ticket_status');
