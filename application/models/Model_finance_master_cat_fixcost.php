<?php
class Model_finance_master_cat_fixcost extends CI_Model {
	...
	{
		$column_order = array(null, 'a.nama', 'hide_expenses'); 
		$order_name = (isset($_POST['order']))? $column_order[$_POST['order']['0']['column']]:'a.name';
		$order_dir = (isset($_POST['order']))? $_POST['order']['0']['dir']:'asc';
		
		$this->db->select("SQL_CALC_FOUND_ROWS a.*, if(a.hide_expenses = 0, '', 'HIDE') as hide_expenses", false);
        $this->db->from('finance_fixcost_cat a');
		$this->db->group_start();
	...
	function insert(){
		$data = array( 
					...
					'hide_expenses' => $this->input->post('hide_expenses'),
				);
		$result=$this->db->insert('finance_fixcost_cat', $data);	
		if($result==true)
		{
			$msg = 1;
		}
	...
	function select(){
		...
		$q = $this->db->get("finance_fixcost_cat");
		return $q->result();
	}
	...
	function update(){
		...
		$this->db->where('id', $this->input->post('id'));
		$result=$this->db->update('finance_fixcost_cat', $data);	
		if($result==true)
		{
			$msg = 1;
		}
	...
	{
		$this->db->where('id', $id);
		$result=$this->db->delete('finance_fixcost_cat');
		if($result==true)
		{
			$msg = 1;
		}
