<?php
class Model_finance_ap_report extends CI_Model
{
	...
	function get_data_table()
	{
		...
		IF(a.supplier = 0, 'Lain2', b.nama) AS nama_supplier,
		(a.jumlah - a.bayar) AS piutang,
		DATEDIFF('" . $tanggal . "', a.date_due) AS aging,
		CASE WHEN a.lunas = 0 THEN 'BELUM LUNAS'
		ELSE 'LUNAS'
		END AS status_inv
	  FROM
	  ...
	  WHERE `a`.`supplier` = '" . $this->input->post('searchkategori') . "' AND a.`status`!=9 $kondisi
		AND (a.tanggal between '" . $this->input->post('searchDateFirst') . "' and '" . $this->input->post('searchDateFinish') . "')
	  GROUP BY `a`.`id`
	  ORDER BY `b`.`nama` ASC,
		`a`.`tanggal` ASC,
		`a`.`no_referensi` ASC ");
	...
	{
		$this->db->where('id', $id);
		$result = $this->db->delete('finance_ap_invoice');
		if ($result == true) {
			$msg = 1;
		} else {
			$msg = 0;
