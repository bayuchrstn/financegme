<?php
class Model_finance_master_cat_forecast extends CI_Model {
	...
	function get_data_table()
	{
        ...
        $this->db->from('finance_forecast_cat a');
		$this->db->group_start();
	...
	function insert(){
		...
		$result=$this->db->insert('finance_forecast_cat', $data);	
		if($result==true)
		{
			$msg = 1;
		}
	...
	function select(){
		...
		$q = $this->db->get("finance_forecast_cat");
		return $q->result();
	}
	...
	function update(){
		...
		$this->db->where('id', $this->input->post('id'));
		$result=$this->db->update('finance_forecast_cat', $data);	
		if($result==true)
		{
			$msg = 1;
		}
	...
	{
		$this->db->where('id', $id);
		$result=$this->db->delete('finance_forecast_cat');
		if($result==true)
		{
			$msg = 1;
		}
