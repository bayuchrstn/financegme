<?php 
class Model_finance_master_cs_acc extends CI_Model {
	...
	{
		$column_order = array(null, 'a.general_ledger'); 
		$order_name = (isset($_POST['order']))? $column_order[$_POST['order']['0']['column']]:'a.name';
		$order_dir = (isset($_POST['order']))? $_POST['order']['0']['dir']:'asc';
		
		$this->db->select("SQL_CALC_FOUND_ROWS a.*, date_format(a.general_ledger, '%d/%m/%Y') as tanggalnya", false);
	...
	function update(){
		$data = array( 
					'general_ledger' => $this->input->post('tanggal'),
				);
		$this->db->where('id', $this->input->post('id'));
