<?php
class Model_finance_master_cat_tax_type extends CI_Model {
	...
	{
		$column_order = array(null, 'a.nama', 'ignore_masukan'); 
		$order_name = (isset($_POST['order']))? $column_order[$_POST['order']['0']['column']]:'a.name';
		$order_dir = (isset($_POST['order']))? $_POST['order']['0']['dir']:'asc';
		
		$this->db->select("SQL_CALC_FOUND_ROWS a.*, if(a.ignore_masukan = 0, '', 'YA') as ignore_masukan", false);
        $this->db->from('finance_master_cat_tax_type a');
		$this->db->group_start();
	...
	function insert(){
		$data = array( 
					...
					'ignore_masukan' => $this->input->post('ignore_masukan'),
				);
		$result=$this->db->insert('finance_master_cat_tax_type', $data);	
		if($result==true)
		{
			$msg = 1;
		}
	...
	function select(){
		...
		$q = $this->db->get("finance_master_cat_tax_type");
		return $q->result();
	}
	...
	function update(){
		...
		$this->db->where('id', $this->input->post('id'));
		$result=$this->db->update('finance_master_cat_tax_type', $data);	
		if($result==true)
		{
			$msg = 1;
		}
	...
	{
		$this->db->where('id', $id);
		$result=$this->db->delete('finance_master_cat_tax_type');
		if($result==true)
		{
			$msg = 1;
		}
